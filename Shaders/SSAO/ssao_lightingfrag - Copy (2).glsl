...
void AddBPLighting(vec3 position, vec3 normal, vec3 albedoCol, int lightIndex, inout vec3 lightResult)
{
	...
	if (lightIndex < numShadowCastingLights)
	{
		...
		if (shadowProj.w > 0.0)
		{
			for (int x = -2; x <= 2; ++x)
			{
				for (int y = -2; y <= 4; ++y)
				{
					shadow += textureProj(shadows[lightIndex], shadowProj + vec4(vec2(x, y)/* * texelSize*/, 1.0f, 1.0f));
				}
			}
		}
...
void main(void){
	...
	vec3 albedoCol = texture(gAlbedo, TexCoords).rgb; //vec3(0.75f, 0.75f, 0.75f);// vec3(0.1, 0.1, 0.1);//
