...
OGLRenderer::OGLRenderer(Window &window)	{
	...
	if(!debugDrawingRenderer) {
		...
		debugSphereShader	 = new Shader(SHADERDIR"/Debug/modelDebugVert.glsl", SHADERDIR"/Debug/modelDebugFrag.glsl");
		orthoDebugData		 = new DebugDrawData();
		perspectiveDebugData = new DebugDrawData();
		debugDrawingRenderer = this;	
		
		if(!debugDrawShader->LinkProgram() || !debugSphereShader->LinkProgram()) {
			return;
		}
	}
}
...
void	OGLRenderer::DrawDebugOrtho(Matrix4*matrix) {
	...
	if(matrix) {
		...
		static Matrix4 ortho = Matrix4::Orthographic(-1,1,1280,0,0,720);
